...
const HACKERAI_DESKTOP_USER_AGENT_TOKEN = "ZHACKER-Desktop";

export const LEGACY_DESKTOP_AGENT_UPDATE_MESSAGE =
  "Agent mode now requires the latest ZHACKER Desktop app. Please update ZHACKER Desktop, then try again.";

export function isZHACKERDesktopUserAgent(
...
export function isLegacyDesktopAgentClient({
  ...
  return mode === "agent" && isTauri && !isZHACKERDesktopUserAgent(userAgent);
}
