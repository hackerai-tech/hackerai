...
export type AgentTaskPayload = {
  ...
  sandboxFiles?: Array<{ path: string; content: string; language?: string }>;
  fileTokens: number;
 ...
 * Call this from POST /api/agent only when mode === "agent".
 */
export async function prepareAgentPayload(
  ...
  if (mode !== "agent") {
    throw new ChatSDKError(
      "bad_request:api",
      "prepareAgentPayload is only for agent mode",
    );
  }
