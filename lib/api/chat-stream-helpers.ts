...
export function sendRateLimitWarnings(
  ...
  } else if (rateLimitInfo.limit > 0) {
    // Paid users: warn when extra usage is being used
    if (
      rateLimitInfo.extraUsageAmountDeducted &&
      rateLimitInfo.extraUsageAmountDeducted > 0
    ) {
      writeRateLimitWarning(writer, {
      ...
      // Warn when remaining budget is below 10%
