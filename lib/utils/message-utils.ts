...
/**
 * Represents a saved file extracted from tool outputs
 */
...
/**
 * Extract saved files from a message's tool outputs.
 * Only extracts files from get_terminal_files tool.
 */
export const extractSavedFilesFromMessage = (message: {
  parts?: Array<any>;
}): Array<SavedFile> => {
  const files: Array<SavedFile> = [];
  ...
  for (const part of parts) {
    // Only extract files from get_terminal_files tool
    if (
      ...
      // Check if output contains fileUrls array
      if (Array.isArray(output?.fileUrls)) {
        for (const file of output.fileUrls) {
          if (file?.path && file?.downloadUrl) {
