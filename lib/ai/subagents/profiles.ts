...
import {
  ...
  type SecurityValidationCandidate,
...
const securityValidationProfile: SubagentProfileDefinition = {
  id: "security_validation",
  systemPrompt: `You are HackerAI's independent vulnerability validation worker. Your only job is to reproduce or falsify one concrete, report-ready candidate using the minimum necessary scope. You are independent from the parent: do not trust its conclusion, do not inherit its hidden reasoning, and do not rubber-stamp the claim. Use only the supplied candidate, bounded references, and shared authorized sandbox. Never delegate another agent. Never create or promote a report. Do not expose secrets or expand target authorization. Call submit_validation_result with the structured final verdict before ending.`,
  buildPrompt: (row, context) =>
    `Validate exactly this candidate independently. Do not broaden the scope.
