...
for every authenticated user already authorized to use that model. Assignment
is stable by authenticated user ID and applies to text-only Ask and Agent
requests whose resolved initial model is `model-grok-4.6`:
...
The experiment excludes images and image tool results, moderation/Abliteration
reroutes, paid allowance rescue, and subagent-only routing. Upstream
authorization and model-access checks remain authoritative; the experiment does
not grant access to Max. Fallbacks after exposure remain attributed by intention
to treat and are reported as a guardrail. Existing billing, rate limits, safety
gates, prompts, tools, and model recovery chains remain authoritative. Missing,
inactive, invalid, or unavailable flag evaluation retains Grok 4.6.
...
Both experiments are running and their flags are active. The custom exposure
criterion is `ultra_max_model_experiment_exposed`. Changes to code-level
eligibility require both Vercel and Trigger deployments; flag-only routing
changes affect new requests and Agent runs without another deployment.
...
- On the actual Preview URL and Trigger Preview worker, use separate eligible
  accounts assigned to each arm. Complete disposable Ask and Agent Max
...
- Verify image requests, unauthorized Max requests, Abliteration-eligible
  requests, allowance rescue, and blocked/pre-start canceled requests do not
  enter or expose.
- Exercise fallback, abort, Agent approval/resume, and a disabled Preview flag.
- Before any Production rollout change, read back both experiment/flag
  definitions and attach runtime identity and acceptance evidence to HAC-127.
