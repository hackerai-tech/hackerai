...
import { stripTriggerChatHeartbeatParts } from "@/lib/chat/trigger-chat-heartbeat";
import { FREE_AGENT_LONG_RUN_LOCK_TTL_SECONDS } from "@/lib/rate-limit/free-config";
import {
  getTerminalProviderStreamError,
  isChatNotFoundError,
  isHandledUserRateLimitError,
  isTerminalProviderStreamError,
  recordTriggerChatFailureForDashboard,
  recordTriggerChatHandledRateLimitForDashboard,
} from "./chat-task-dashboard";
import { withTriggerChatStreamHeartbeat } from "./chat-task-stream";
...
export type TriggerChatPayload = {
  chatId: string;
  userId: string;
  mode?: ChatMode;
...
type ChatTriggerTaskOptions = {
  id: string;
  defaultMode: ChatMode;
};

type ChatTriggerRunContext = {
  ctx: { run: { id: string } };
  signal: AbortSignal;
};

async function runChatTriggerTask(
  payload: TriggerChatPayload,
  { ctx, signal: triggerSignal }: ChatTriggerRunContext,
  options: ChatTriggerTaskOptions,
) {
  const taskId = options.id;
  const logPrefix = `[${taskId}]`;
  ...
  const mode: ChatMode = payloadMode ?? options.defaultMode;
  const endpoint = isAgentMode(mode) ? "/api/agent" : "/api/chat";
  ...
  try {
        ...
        try {
          ...
          if (
            ...
            try {
              ...
              console.warn(`${logPrefix} Failed to get sandbox context:`, err);
            }
          }
          ...
          const streamCtx: AgentStreamContext = {
            ...
            streamStartTime,
            contextUsageOn,
            isReasoningModel: isAgentMode(mode),
          ...
          try {
            ...
            if (
              ...
              phLogger.error(
                `${logPrefix} Provider API error, retrying with fallback`,
                {
                  taskId,
                  error,
                  chatId,
          ...
          writer.merge(
            withTriggerChatStreamHeartbeat(
                    ...
                    if (!temporary) {
                      ...
                      if (incompleteToolSummaries.length > 0) {
                        console.info(
                          JSON.stringify({
                            level: "info",
                            event:
                              "trigger_chat_abort_incomplete_tool_calls_detected",
                            service: taskId,
                      ...
                      if (
                        ...
                        console.info(
                          JSON.stringify({
                            level: "info",
                            event: "trigger_chat_abort_message_save_skipped",
                            service: taskId,
                            timestamp: new Date().toISOString(),
                            task_id: taskId,
                            chat_id: chatId,
                      ...
                      for (const message of finishedMessages) {
                        const processed = stripTriggerChatHeartbeatParts(
    ...
    if (terminalStreamError) {
      if (isHandledUserRateLimitError(terminalStreamError)) {
        await recordTriggerChatHandledRateLimitForDashboard(
          terminalStreamError,
          {
            taskId,
            chatId,
          ...
          console.error(
            `${logPrefix} failed to record rate limit metadata:`,
            metadataError,
          );
        });
    ...
    await recordTriggerChatFailureForDashboard(error, {
      taskId,
      chatId,
      userId,
      runId: ctx.run.id,
      ...
      console.error(
        `${logPrefix} failed to record run error metadata:`,
        metadataError,
      );
    });
    await usageRefundTracker.refund().catch(() => {});
    ...
    await ptySessionManager
      .closeAll(chatId)
      .catch((err) =>
        console.error(`${logPrefix} PTY closeAll (outer catch) failed:`, err),
      );
    ...
    if (!streamPiped) {
      try {
        ...
        console.error(
          `${logPrefix} Failed to emit synthetic error stream:`,
          pipeErr,
        );
      }
    }
    ...
    if (!payload.temporary) {
      try {
        ...
        console.error(
          `${logPrefix} failed to clear active_trigger_run_id:`,
          error,
        );
      }
    }
...
export const createChatTriggerTask = (options: ChatTriggerTaskOptions) =>
  task({
    id: options.id,
    maxDuration: 60 * 60,
    ...
    run: async (payload: TriggerChatPayload, runContext) =>
      runChatTriggerTask(payload, runContext, options),
  });
