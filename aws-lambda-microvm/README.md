...
the eight-hour maximum lifetime. Data Controls **Delete sandboxes** and account
deletion both remove the durable object from all three regions. Presigned
transfer URLs are generated by service-key-authenticated Convex actions using
the existing `AWS_S3_ACCESS_KEY_ID` and `AWS_S3_SECRET_ACCESS_KEY` plus these
workspace-only Convex environment variables:

```text
AWS_LAMBDA_MICROVM_WORKSPACE_BUCKET_US_EAST_1=<private bucket in us-east-1>
AWS_LAMBDA_MICROVM_WORKSPACE_BUCKET_US_WEST_2=<private bucket in us-west-2>
AWS_LAMBDA_MICROVM_WORKSPACE_BUCKET_EU_WEST_1=<private bucket in eu-west-1>
```
