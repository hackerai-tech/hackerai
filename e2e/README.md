...
2. **Wait for elements**: Use Playwright's auto-waiting, don't add arbitrary timeouts
3. **Clean up**: Clear auth cache between tests to ensure isolation
4. **Handle rate limits**: Use session caching, don't bypass it unless testing login flow specifically
5. **Test real flows**: E2E tests use real WorkOS and Convex, not mocks
6. **Verify state**: Always verify both visual state (UI) and logical state (URLs, cookies)
